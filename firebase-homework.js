...
function saveTaskDone(taskId, taskInfo) {
    ...
    db.ref('done/' + currentUser + '/' + taskId).set(now.getTime());
...
async function getDoneIds() {
    ...
    for (const [id, val] of Object.entries(data)) {
        // backward compat: old entries saved as true get timestamp 0 (always show as done)
        map.set(id, typeof val === 'number' ? val : 0);
    }
    return map;
}
...
async function renderHomework(grade, containerId) {
    ...
    const doneMap = await getDoneIds();
    const now = Date.now();
    const HIDE_AFTER_MS = 3 * 24 * 60 * 60 * 1000; // 3 days

    const gradeTasks = tasks.filter(t => t.grade === grade);

    // Split into pending and recently-done (hide tasks done > 3 days ago)
    const pendingTasks = [];
    const doneTasks = [];

    gradeTasks.forEach(t => {
        if (doneMap.has(t.id)) {
            const doneAt = doneMap.get(t.id);
            // Hide if done more than 3 days ago (or old entries with timestamp 0)
            if (doneAt === 0 || (now - doneAt) > HIDE_AFTER_MS) return;
            doneTasks.push(t);
        } else {
            pendingTasks.push(t);
        }
    ...
    doneTasks.forEach(task => {
        const dateHtml = task.date ? `<span class="hw-date">${task.date}</span>` : '';
        html += `
            <div class="homework-item homework-done" id="task-${task.id}">
                <div class="homework-info" style="width: 100%;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                        <span class="homework-subject">${task.subject}</span>
                        ${dateHtml}
                    </div>
                    <div style="color:#555; line-height:1.4;"><s>${task.task}</s></div>
                </div>
                <span class="hw-done-check">✔</span>
            </div>
        `;
    });
...
window.markHomeworkDone = function(id) {
    ...
    if (taskItem) {
        // Animate to done state with strikethrough
        taskItem.style.transition = "all 0.5s ease";
        taskItem.classList.add("homework-done");

        // Strikethrough the task text
        const taskTextDiv = taskItem.querySelector('.homework-info > div:last-child');
        if (taskTextDiv) {
            taskTextDiv.innerHTML = '<s>' + taskTextDiv.innerHTML + '</s>';
        }
